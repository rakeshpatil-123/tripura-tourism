...
export class LineOfActivityComponent implements OnInit, OnDestroy {
  ...
  isDeleting = false;
  deletingRowIndex: number | null = null; 
  form: FormGroup;
  ...
  unitOptions: SelectOption[] = [
    ...
    {
      id: 'Metric Tonnes Numbers Per Month',
      name: 'Metric Tonnes Numbers Per Month',
    },
    {
      id: 'Kilo Liters Number Per Month',
      name: 'Kilo Liters Number Per Month',
    },
    { id: 'Meter Numbers Per Month', name: 'Meter Numbers Per Month' },
    {
      id: 'Square Meter Numbers Per Month',
      name: 'Square Meter Numbers Per Month',
    },
    {
      id: 'Cubic Meter Numbers Per Month',
      name: 'Cubic Meter Numbers Per Month',
    },
    { id: 'Foot Numbers Per Month', name: 'Foot Numbers Per Month' },
    {
      id: 'Square Foot Numbers Per Month',
      name: 'Square Foot Numbers Per Month',
    },
  ...
  loadExistingData(): void {
    this.apiService
      .getByConditions({}, 'api/caf/line-of-activity-view')
      .subscribe({
        next: (res: any) => {
          if (res && res.status === 1) {
            this.patchFormWithData({
              thrust_sector:
                res.line_of_activity?.thrust_sector || 'None of these',
  ...
  addRawMaterial(data?: any): void {
    const group = this.fb.group({
      ...
      raw_material_quantity_per_month: [
        data?.raw_material_quantity_per_month || '',
        Validators.required,
      ],
  ...
  const rawMaterialGroup = this.rawMaterials.at(index) as FormGroup;
  const id = rawMaterialGroup.get('id')?.value;

  if (id) {
    this.deletingRowIndex = index; 

    this.apiService
      .getByConditions({ id }, 'api/caf/raw-material-delete')
      .subscribe({
        next: () => {
          this.apiService.openSnackBar('Raw material deleted', 'success');
          this.rawMaterials.removeAt(index);
        },
        error: (err) => {
          console.error('Error deleting raw material:', err);
          this.apiService.openSnackBar('Failed to delete raw material', 'error');
        },
        complete: () => {
          this.deletingRowIndex = null; // Re-enable all buttons
        }
      });
  } else {
    this.rawMaterials.removeAt(index);
  ...
  addProduct(data?: any): void {
    const group = this.fb.group({
      ...
      product_production_capacity_per_month: [
        data?.product_production_capacity_per_month || '',
        Validators.required,
      ],
      product_average_production_per_month: [
        data?.product_average_production_per_month || '',
        Validators.required,
      ],
...
removeProduct(index: number): void {
  ...
  if (id) {
    ...
    this.apiService
      .getByConditions({ id }, 'api/caf/list-of-products-delete')
      .subscribe({
        next: () => {
          this.apiService.openSnackBar('Product deleted', 'success');
          this.products.removeAt(index);
        },
        error: (err) => {
          console.error('Error deleting product:', err);
          this.apiService.openSnackBar('Failed to delete product', 'error');
        },
        complete: () => {
  ...
  private submitForm(payload: any, isDraft: boolean): void {
    this.apiService
      .getByConditions(payload, 'api/caf/line-of-activity-store')
      .subscribe({
        next: (res) => {
