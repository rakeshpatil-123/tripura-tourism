...
@Component({
  selector: 'app-renewal-application-submission',
  imports: [
    IlogiInputComponent,
    IlogiRadioComponent,
    IlogiSelectComponent,
  ...
  templateUrl: './renewal-application-submission.component.html',
  styleUrl: './renewal-application-submission.component.scss',
})
export class RenewalApplicationSubmissionComponent {
  formModifiedAfterFeeCalculation = false;
  showConfirmModal = false;
  modalConfig = {
    title: 'Confirm Submission',
    message: 'Are you sure you want to renew this application?',
  ...
  ngOnInit(): void {
    this.serviceId = Number(this.route.snapshot.paramMap.get('sId'));
    const appid = Number(this.route.snapshot.paramMap.get('appId'));
    const cId = Number(this.route.snapshot.paramMap.get('renID'));
    if (appid) {
    ...
    this.questions.forEach((q) => {
      ...
      if (q.validation_required === 'yes' && q.validation_rule) {
        ...
        if (q.question_type === 'number') {
          ...
          validators.push(
            RenewalApplicationSubmissionComponent.digitLengthValidator(min, max)
          );
        } else {
          if (rule.minLength != null && rule.minLength !== '') {
  ...
  private submitWithFiles(userId: string, raw: any): void {
    ...
    formData.append('renewal_cycle_id', this.cycleId!.toString());
    const actualAppId = this.applicationId;
    if (actualAppId !== null) {
    ...
    this.apiService
      .getByConditions(formData, 'api/user/update-renewed-application')
      .subscribe({
        next: (res) => {
          if (res?.status === 1) {
  ...
  private prepareApplicationData(raw: any): any {
    const appData: any = {};

    this.questions.forEach((q) => {
      const value = raw[q.id];
      if (value != null || q.is_required === 'yes') {
        appData[q.id] = value;
      }
    });

    this.sectionGroups.forEach((section) => {
      const sectionRows = raw[section.sectionName] || [];
      const validRows = sectionRows.filter((row: any) =>
      ...
      if (validRows.length > 0) {
        appData[section.sectionName] = validRows.map((row: any) => {
          const rowObj: any = {};
          section.questions.forEach((q) => {
            rowObj[q.id] = row[q.id];
          });
          return rowObj;
        });
      }
  ...
  calFee(): void {
    ...
    const raw = this.serviceForm.getRawValue();
    const applicationData = this.prepareApplicationData(raw);

    const payload = new FormData();
    payload.append('application_id', this.applicationId!.toString());
    payload.append('renewal_cycle_id', this.cycleId!.toString());

    Object.keys(applicationData).forEach((key) => {
      const value = applicationData[key];
      if (value instanceof File) {
        payload.append(`application_data[${key}]`, value, value.name);
      } else {
        payload.append(`application_data[${key}]`, value ?? '');
      }
    });

    this.apiService
      .getByConditions(payload, 'api/user/calculate-renewal-fee')
      .subscribe({
        next: (res: any) => {
          if (res?.status === 1) {
            this.calculatedFee = Number(res.data.final_fee.renewal_fee);
            // this.effectiveFee = Number(res.data.final_fee.effective_fee);
            // this.late_fee = Number(res.data.final_fee.late_fee);
            this.visible = true;
