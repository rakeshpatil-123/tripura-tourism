...
//           label: 'Confirm Date',
//           color: 'primary',
//           visible: (row: any) => row.status === "date_changed",
    ...
    { key: 'updated_by', label: 'Updated By', type: 'text' },
    {
      ...
      actions: [
        ...
        },
        {
          ...
          color: 'primary',
          visible: (row: any) =>
            row.status !== 'approved' && row.status !== 'rejected' && row.status !== 'Date Changed' ,
          onClick: (row: any) => {
            this.openEditModal(row);
          },
        },
  ...
  confirmDate(): void {
    ...
    this.genericService
      ...
      .subscribe({
        next: (res: any) => {
          if (res?.status === 1) {
            this.genericService.openSnackBar('Date confirmed successfully!', 'success');
            this.closeConfirmDateDialog();
            this.loadInspectionLists();
          } else {
            this.genericService.openSnackBar(res?.message || 'Failed to confirm date', 'error');
          }
        },
        error: (err) => {
  ...
  submitChangedDate(): void {
    ...
    this.genericService
      ...
      .subscribe({
        next: (res: any) => {
          if (res?.status === 1) {
            this.genericService.openSnackBar('Date updated successfully!', 'success');
            this.closeConfirmDateDialog();
            this.loadInspectionLists();
          } else {
            this.genericService.openSnackBar(res?.message || 'Failed to update date', 'error');
          }
        },
        error: (err) => {
