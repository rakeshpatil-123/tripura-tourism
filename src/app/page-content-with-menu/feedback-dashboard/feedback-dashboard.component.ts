...
// Standalone child components you provided earlier (keep in imports if they exist as standalone)
import { FilterBarComponent } from '../filter-bar/filter-bar.component';
import { IlogiSelectComponent, SelectOption } from '../../customInputComponents/ilogi-select/ilogi-select.component';

// Services (use your implementations)
...
export class FeedbackDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  get totalPages() {
    return Math.max(1, Math.ceil((this.filteredFeedbacks?.length || 0) / this.pageSize));
  }
  get totalPagesArray() {
  ...
  onDepartmentSelect(deptId: number | null) {
    ...
    this.fetchDepartmentFeedbacks(deptId);
  }

  fetchDepartmentFeedbacks(department_id: number) {
    this.loaderService.showLoader();
    this.isLoading = true;
    const s = this.genericService.getByConditions({ department_id: department_id }, 'api/user-feedback-details')
      .pipe(finalize(() => { this.loaderService.hideLoader(); this.isLoading = false; }))
      .subscribe({
        next: (res: any) => {
          if (res?.status) {
            this.summary = res.summary || this.summary;
            this.feedbacks = Array.isArray(res.data) ? res.data : [];
            this.applyFiltersAndRefreshCharts();
  ...
  exportExcel() {
    ...
    const headers = ['#', 'User', 'Submitted On', 'Rating', 'Feedback', 'Suggestions'];
